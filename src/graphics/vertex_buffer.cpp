...
void VertexBuffer::drawFullScreenImageEffect() {
    glDisable(GL_DEPTH_TEST);
    glDisable(GL_CULL_FACE);
    if (fullScreenImageQuad == nullptr) {
        fullScreenImageQuad = SharedMesh(Mesh::quad());
        VertexBuffer::with(VA_POSITION)->add(fullScreenImageQuad)->upload();
    }
    fullScreenImageQuad->render();
...
void VertexBuffer::upload()
{
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int) * totalNrOfIndicies, NULL, GL_STATIC_DRAW);
    ...
    for (const std::weak_ptr<Mesh>& m : meshes)
    {
        ...
        std::cout << "VertexBuffer "<< vaoId << " adding mesh: verts" << m.lock()->nrOfVerts << "   vertsize " << vertSize << std::endl;

        SharedMesh mesh = m.lock();

        GLuint meshVertSize = mesh->nrOfVerts * sizeof(float) * vertSize;
        glBufferSubData(GL_ARRAY_BUFFER, vertsOffset, meshVertSize, mesh->vertices.data());
        vertsOffset += meshVertSize;

        GLuint meshIndicesSize = mesh->nrOfIndices * sizeof(int);
    ...
    glBufferSubData_test(GL_ELEMENT_ARRAY_BUFFER, sizeof(int) * totalNrOfIndicies);
#endif
    ...
    for (int i = 0; i < vertAttributes.nrOfAttributes(); i++) {
        ...
        glVertexAttribPointer(
            ...
            vertSize * sizeof(GL_FLOAT),     // stride
