...
Texture::Texture(const char *imgPath, GLint textureWrapping, GLint textureInterpolation, GLint format)
{
    ...
    if (imgPath == nullptr)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, format, 2, 2, 0, format, GL_FLOAT, pixels);
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, format, GL_UNSIGNED_BYTE, imagePtr);
